...
class TestUploadPartCopySSECHappyPath:
    ...
    # =========================================================================
    # Test 4: SSE-C source -> SSE-C destination (different key = rejected)
    # =========================================================================

    @pytest.mark.usefixtures("setup_test_bucket")
    @pytest.mark.edge_case
    def test_copy_ssec_to_ssec_new_key_rejected(
        self, test_bucket, ssec_source, ssec_multipart_upload,
        make_request, json_metadata, request,
    ):
        """UploadPartCopy rejects different dest SSE-C key than MPU was created with.

        Unlike CopyObject, UploadPart(Copy) requires the same SSE-C key as
        CreateMultipartUpload. AWS returns 400 InvalidRequest:
        ...
        headers = {
            ...
            # Dest SSE-C headers (encrypt with DIFFERENT key — rejected by AWS)
            "x-amz-server-side-encryption-customer-algorithm": "AES256",
